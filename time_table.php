<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
  
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
    <link rel="stylesheet" href="style.css">
</head>
<body>


<div class="row">
    <div class="col p10">
    <div class="card">
    <h3 class="card-header mb-5 text-uppercase bg-info">CLASS TIME TABLE - 2021</h3>
    <div class="card-body">
       
    <div class="table-responsive">
    <table class="table table-bordered table-dark col-xl">
    <thead>
    <tr>
      <th scope="col">Time</th>
      <th scope="col">Monday</th>
      <th scope="col">Tuesday</th>
      <th scope="col">Wednesday</th>
      <th scope="col">Thursday</th>
      <th scope="col" rowspan=9>Friday</th>
      <th scope="col">Saturday</th>
      <th scope="col">Sunday</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <th scope="row">08.30-09.30</th>
      <td>DBS II - MIII</td>
      <td>WP-T- AJMH</td>
      <td>LAN-T- AJMH</td>
      <td>ST-T- SR</td>
      <td> </td>
      <td> - </td>
      <td>ST-T- SR</td>
    </tr>

    <tr>
      <th scope="row">09.30 - 10.30</th>
      <td>DBS II - MIII</td>
      <td>WP-T- AJMH</td>
      <td>LAN-T- AJMH</td>
      <td>ST-T- SR</td>
      <td> </td>
      <td> - </td>
      <td>ST-T- SR</td>
    </tr>

    <tr>
      <th scope="row">10.30 - 11.30</th>
      <td>DBS II - MIII</td>
      <td>WP-P- AJMH</td>
      <td>LAN-T- AJMH</td>
      <td>PSW-T- MIII</td>
      <td> </td>
      <td> - </td>
      <td>ST-T- SR</td>
    </tr>

    <tr>
      <th scope="row">11.30 - 12.30</th>
      <td>DBS II - MIII</td>
      <td>WP-P- AJMH</td>
      <td>LAN-P- AJMH</td>
      <td>PSW-T- MIII</td>
      <td> </td>
      <td> - </td>
      <td>ST-P- SR</td>
    </tr>

    <tr>
      <th scope="row">12.30 - 01.00</th>
      <td colspan=7 style="text-align:center;">INTERVAL</td>
      
    </tr>

    <tr>
      <th scope="row">1.00 - 02.00</th>
      <td>ST-T- SR</td>
      <td>ST-T- SR</td>
      <td>LAN-P- AJMH</td>
      <td>PSW-T- MIII</td>
      <td> </td>
      <td> ST-T- SR </td>
      <td>ST-P- SR</td>
    </tr>

    <tr>
      <th scope="row">02.00 - 03.00</th>
      <td>ST-T- SR</td>
      <td>ST-T- SR</td>
      <td>WP-P- AJMH</td>
      <td></td>
      <td> </td>
      <td> ST-T- SR </td>
      <td>ST-P- SR</td>
    </tr>

    <tr>
      <th scope="row">03.00 - 04.00</th>
      <td>ST-T- SR</td>
      <td>ST-T- SR</td>
      <td></td>
      <td></td>
      <td> </td>
      <td> ST-T- SR </td>
      <td></td>
    </tr>

    <tr>
      <th scope="row">04.00 - 05.00</th>
      <td>ST-T- SR</td>
      <td>ST-T- SR</td>
      <td></td>
      <td></td>
      <td> </td>
      <td> ST-T- SR </td>
      <td></td>
    </tr>
  </tbody>
</table>
</div>

    </div>
    </div>
    </div>
</div>



<script src="https://code.jquery.com/jquery-3.4.1.slim.min.js" integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js" integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6" crossorigin="anonymous"></script>
</body>
</html>